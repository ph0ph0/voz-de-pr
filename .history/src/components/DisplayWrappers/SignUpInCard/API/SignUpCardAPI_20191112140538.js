...
const SignUpInCardAPI = ({ state, setState }) => {
  ...
  const updateFirstEmailValue = newValue => {
    setState(prevState => {
      return {
        ...
        emailsMatch: emailsAreSame(newValue, secondEmailValue) ? true : false
      };
    });
  ...
  const updateSecondEmailValue = newValue => {
    setState(prevState => {
      return {
        ...
        emailsMatch: emailsAreSame(newValue, firstEmailValue) ? true : false
      };
    });
    window.log(`emailValue: ${newValue}`);
  };
  ...
  //Checks that the newly entered second email matches the first email. Used to set emailsMatch bool in updateSecondEmail
  const emailsAreSame = (newValue, otherValue) => {
    return newValue === otherValue;
  };

  //Checks that the newly entered second password matches the first email. Used to set passwordsMatch bool in updatePasswordEmail
